...
export class FiltreSeriePipe implements PipeTransform {

  public transform(values: any[], filtre: number): any[] {
    // console.log(values[0], filtre);
    if (!values || !values.length) return [];
    if (!filtre) return values;
    ...
    return values.filter(v => {
      // Filtre sur le titre
      if(v.ISBN13){
        console.log(v.ISBN13 == filtre, v.ISBN13, filtre);
        return v.ISBN13 == filtre;
      }
    });
  }
